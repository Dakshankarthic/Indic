"""
PyTorch Dataset for Custom U-Net
Loads images and ground-truth masks for training the model.
"""
import os
import torch
import cv2
...
class ManuscriptDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask = mask_data['mask'] # Shape: (H, W, 3)
        
        # Convert pixel values [0, 255] to binary [0.0, 1.0]
        mask = (mask > 127).astype(np.float32)

        # Basic transforms (Resize to 512x512 for training simplicity if needed)
        # Note: A real implementation uses albumentations for random crops/flips
...
if __name__ == '__main__':
    # Test dataset
    ds = ManuscriptDataset(r"D:\indic_challenge\training_data\images", r"D:\indic_challenge\training_data\masks")
    if len(ds) > 0:
        img, mask = ds[0]
