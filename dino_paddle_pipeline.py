import os
# Disable oneDNN to avoid PaddlePaddle C++ execution bug on Windows CPU
...
def cluster_text_mask(feat_grid, img_bgr, n_clusters=3):
    ...
    # Identify the text cluster based on highest variance (usually text)
    variances = []
...
# ─── 3. PADDLE OCR INFERENCE ────────────────────────────────────────

def process_paddle_ocr_on_regions(ocr, img, regions):
    lines_data = []
    num_words = 0
    ...
    for r_idx, (rx, ry, rw, rh) in enumerate(regions, 1):
        ...
        for line in result[0]:
            ...
            # Gap factor to put a small gap between underlines
            gap = 0.05
            
            for wi, w_text in enumerate(words_text):
                # Calculate word baseline segment with a small gap
...
def generate_pagexml(img_name, img_w, img_h, lines_data, out_path):
    ...
    etree.SubElement(metadata, "Creator").text = "DINOv2 + PaddleOCR Pipeline"
...
def process_image(dino_model, ocr_model, img_path, output_dir):
    ...
    # Step 2: PaddleOCR lines and text
    lines_data, num_words = process_paddle_ocr_on_regions(ocr_model, img, regions)
...
def main():
    ...
    ocr_model = PaddleOCR(use_textline_orientation=True, lang='hi')
    ...
    for img_path in pbar:
        try:
            lines_count, words_count = process_image(dino_model, ocr_model, img_path, out_path)
