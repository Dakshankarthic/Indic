...
def train():
    ...
    parser.add_argument('--data_dir', type=str, default=str(base_dir / 'training_data_zero_shot'))
    ...
    if len(full_dataset) == 0:
        print("No training data found. Please run run_zero_shot_label_pipeline.py first.")
        return
    ...
    model = UNet(n_channels=3, n_classes=6, deep_supervision=True)
    ...
    for epoch in range(args.epochs):
        ...
        for images, masks in pbar:
                    ...
                    if isinstance(outputs, list):
                        ...
                        for i, out in enumerate(outputs):
                            if out.shape[2:] != masks.shape[2:]:
                                mask_ds = F.interpolate(masks, size=out.shape[2:], mode='nearest')
                            else:
                                mask_ds = masks
                            loss += weights[i] * criterion(out, mask_ds)
                    else:
            ...
            train_loss += loss.item() if not isinstance(outputs, list) else loss.item() / sum(weights)
